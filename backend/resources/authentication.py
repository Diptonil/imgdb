...
class Login(Resource):
    ...
    def post(self):
        ...
        with self.database_driver.session() as session:
            result = session.run(CHECK_USER, map).single()
...
class Register(Resource):
    ...
    def post(self):
        ...
        map = {'id': str(uuid.uuid4()), 'username': username, 'password': password, 'date_of_birth': date_of_birth, 'avatar': avatar}
        result = is_password_strong(password)
        if not result[0]:
            return ({'status': 'Registration unsuccessful. ' + result[1]}, 400)
        with self.database_driver.session() as session:
            if session.run(CHECK_USERNAME, map).single() is None:
                return ({'status': 'Registration unsuccessful. Username already exists'}, 400)
            session.run(CREATE_USER, map)
